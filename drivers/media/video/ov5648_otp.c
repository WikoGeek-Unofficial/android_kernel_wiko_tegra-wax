...
static void otp_read(struct ov5648_info *info, unsigned short otp_addr, unsigned char* otp_data)
{
...
* Return      :  0, group index is empty
                ...
                -1, group index error
*******************************************************************************/	
static signed char otp_check_wb_group(struct ov5648_info *info, unsigned char index)
{   
...
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/	
static signed char otp_read_wb_group(struct ov5648_info *info, signed char index)
{
	...
	if (index == -1)
	{
		...
		for (index=0; index<3; index++)
		{
			...
			{
				printk("[OV5648_OTP] read wb from group %d", index);
				break;
			}
		}
...
static int otp_apply_wb(struct ov5648_info *info, unsigned short golden_rg, unsigned short golden_bg)
{
	...
	if (ratio_r != 1 * OTP_MULTIPLE_FAC)
	{
		...
		OV5648MIPI_write_cmos_sensor(info, GAIN_RL_ADDR, gain_r & 0x00ff);
	}
	...
	if (ratio_g != 1 * OTP_MULTIPLE_FAC)
	{
		...
		OV5648MIPI_write_cmos_sensor(info, GAIN_GL_ADDR, gain_g & 0x00ff);
	}
	...
	if (ratio_b != 1 * OTP_MULTIPLE_FAC)
	{
		...
		OV5648MIPI_write_cmos_sensor(info, GAIN_BL_ADDR, gain_b & 0x00ff);
	}
...
int check_truly_otp_mid(struct ov5648_info *info)
{
	...
	for(index = 0; index < 3; index++)
	{
		...
		//mid &= 0x7f;
		printk("[OV5648_OTP] [check_truly_otp_mid] MID == %d, index == %d\n",mid,index);
		if (0x02 == mid){
			printk("[OV5648_OTP] [check_truly_otp_mid] This is truly module.\n");
			break;
		}
	}
...
u32 OV5648_ReadFuseIDFromOTP(struct ov5648_info *info)
{
	...
	for (index=0; index<3; index++)
	{
		if (otp_check_wb_group(info, index) == 2)
		{
			printk("[OV5648_OTP] [OV5648_ReadFuseIDFromOTP]read wb from group %d", index);
			break;
		}
	}
	if (index > 2)
	{
		printk("[OV5648_OTP] [OV5648_ReadFuseIDFromOTP]no group has valid data\n");
		return -1;
	}
	if (index == 0)
	{
	...
	for(i=0; i<4; i++)
	{
		...
		printk("[OV5648_OTP] [OV5648_ReadFuseIDFromOTP]mingji test Temp[%d] == 0x%x\n", i, Temp[i]);
	}
	OV5648MIPI_write_cmos_sensor(info, 0x3d81, 0x00);
	otp_clear(info);

	fuse_id = (u32)(Temp[0]<<24) | (u32)(Temp[1]<<16) | (u32)(Temp[2]<<8) | (u32)(Temp[3]);
	printk("[OV5648_OTP] [OV5648_ReadFuseIDFromOTP]mingji test fuse_id == 0x%x\n", fuse_id);
